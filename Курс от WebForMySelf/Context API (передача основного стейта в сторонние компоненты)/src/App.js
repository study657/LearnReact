...
class App extends Component{
  ...
  render(){
    const cars = this.state.cars.map((car, index) => {
      return <div key={index} style={{width: '400px', margin: '0 auto', paddingTop: '20px'}}>
        <ErrorBoundary>
    ...
    return (
      <div className={Appclass.App}>
        ...
        <button className={Appclass.appButton} onClick={this.showCars.bind(this)}>{this.state.button}</button><br/>
        <button onClick={() => this.setState({clicked: !this.state.clicked})}>Clicker</button>
